...
def main(argv):
    ...
    for line in input_file:
        ...
        if subexpression.connective[0] == '':
            symbolsList[subexpression.symbol[0]] = True
        if subexpression.connective[0].lower() == 'not':
            ...
            and subexpression.subexpressions[0].connective[0] == '':
                symbolsList[subexpression.subexpressions[0].symbol[0]] = False
    ...
    #Negation of statement paramter for the TTEntails function
    negationStatement = '(not '+ statement + ')' 
    
    # Convert statement into a logical expression and verify it is valid
    statement = read_expression(statement)
    counter = [0]
    negationStatement = read_expression(negationStatement, counter)
    ...
    TTEntails(knowledge_base, statement, negationStatement, symbolsList)
